...
MongoClient.connect(config.get('mongodb')).then(db => {
  ...
  logs.find({ reads: { $elemMatch: { reads: { $elemMatch: { name: "質量流率", time: { $lt: new Date('2017-10-19T11:00:50.000Z') } } } } } }).forEach((doc) => {
    const operation = {
      updateOne: {
        filter: { _id: doc._id },
        update: {
    ...
    _.forEach(doc.reads, (rtu, i) => {
      _.forEach(rtu.reads, (reg, j) => {
        if (!rtu || ! reg || !rtu.name || !reg.name) {
          ...
          if (reg.name === '質量流率') {
            operation.updateOne.update.$set[`reads.${i}.reads.${j}.value`] = reg.value * 3.6
            if (reg.unit === 'kg/h') {
              operation.updateOne.update.$set[`reads.${i}.reads.${j}.value`] = null
              operation.updateOne.update.$set[`reads.${i}.reads.${j}.unit`] = 't/h'
            }
          }
        }
      })
