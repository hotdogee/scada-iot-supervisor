...
MongoClient.connect(config.get('mongodb')).then(db => {
  ...
  // keep  { unit: 'kg' }, { unit: 'm3' } 累積質量 累積體積
  logs.find({ reads: { $elemMatch: { reads: { $elemMatch: { $and: [ { value: { $gt: 30000 } }, { $or: [ { unit: '' }, { unit: 'kW' } ] } ] } } } } }).forEach((doc) => {
    const operation = {
      updateOne: {
        filter: { _id: doc._id },
        update: {
    ...
    _.forEach(doc.reads, (rtu, i) => {
      _.forEach(rtu.reads, (reg, j) => {
        if (!rtu || ! reg || !rtu.name || !reg.name) {
          ...
          if (reg.value > 30000 || reg.value === -1 || (reg.name === '溫度' && reg.value > 300)) {
            operation.updateOne.update.$set[`reads.${i}.reads.${j}.value`] = null
          }
          if (reg.unit === '°C') {
            operation.updateOne.update.$set[`reads.${i}.reads.${j}.unit`] = '℃'
          }
        }
      })
    })
