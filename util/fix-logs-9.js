...
MongoClient.connect(config.get('mongodb')).then(db => {
  ...
  logs.find({ reads: { $elemMatch: { reads: { $elemMatch: { $type: "null" } } } } }).forEach((doc) => {
    const operation = {
      updateOne: {
        filter: { _id: doc._id },
        update: {
    ...
    _.forEach(doc.reads, (rtu, i) => {
      _.forEach(rtu.reads, (reg, j) => {
        if (!reg) {
          console.log(doc._id)
          operation.updateOne.update.$pull.reads = { addr: rtu.addr}
        }
      })
    })
    // console.log(JSON.stringify(operation, null, 2))
    if (!_.isEmpty(operation.updateOne.update.$pull)) {
...
// TODO: 修正M13壓力>8
// TODO: value < 0
// TODO: remove null reg
// TODO: 移除錯誤 M13壓力 logTime 2017-08-08T15:40:22.634Z to 2017-08-10T08:00:23.344Z
// TODO: 移除錯誤 M14溫度 logTime before 2017-08-10T04:09:26.329Z
// TODO: 移除錯誤 M10溫度 logTime before 2017-08-10T04:09:26.329Z
