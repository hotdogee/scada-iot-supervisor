...
MongoClient.connect(mongodb).then(async db => {
  const from = new Date('2017-10-17T21:18:55.409Z') // 2017-10-31 06:08:55.118
  ...
  logs.find(logTimeFilter).sort({ logTime: 1 }).forEach((doc) => {
    ...
    _.forEach(doc.reads, rtu => {
      _.forEach(rtu.reads, reg => {
        ...
        } else if (reg.name === '溫度' && reg.value > 400) {
          // Ignore 溫度 > 300
        ...
        } else if (rtu.addr === 13 && reg.name === '溫度' && reg.time >= new Date('2017-11-03T02:28:22.000Z') && reg.time <= new Date('2017-11-04T11:16:40.000Z')) {
